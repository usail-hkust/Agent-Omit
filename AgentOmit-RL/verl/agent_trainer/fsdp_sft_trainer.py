...
class FSDPSFTTrainer(object):
    ...
    def save_checkpoint(self, step):
        ...
        if self.config.trainer.storage_mode == 'aistudio':
            ...
            save_dir = ckpter.get_save_dir(step=step)
            path = os.path.join(save_dir, f'global_step_{step}')
    ...
    def fit(self):
        ...
        if self.config.trainer.total_epochs > 1 and self.config.trainer.storage_mode == 'aistudio':
