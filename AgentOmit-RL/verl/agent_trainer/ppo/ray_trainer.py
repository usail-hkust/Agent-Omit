...
class RayPPOTrainer(object):
    ...
    def __init__(self,
        ...
        # Fix the assertion issue by checking if ActorRollout role exists in mapping
        self.hybrid_engine = config.actor_rollout_ref.hybrid_engine
        print(f"[DEBUG] hybrid_engine: {self.hybrid_engine}")
        print(f"[DEBUG] role_worker_mapping keys: {list(role_worker_mapping.keys())}")
        
        # Check if ActorRollout role exists using a more robust approach
        actor_rollout_exists = any(role == Role.ActorRollout for role in role_worker_mapping.keys())
        print(f"[DEBUG] ActorRollout exists in mapping: {actor_rollout_exists}")
        
        if self.hybrid_engine:
            if not actor_rollout_exists:
                raise ValueError(f'Hybrid engine requires Role.ActorRollout in role_worker_mapping, but got {list(role_worker_mapping.keys())}')
        else:
            raise NotImplementedError('Currently, only hybrid engine is supported')
    ...
    def _create_dataloader(self):
        ...
        for batch in self.train_dataloader:
            ...
            print("Batch shape/size:", {k: v.shape for k, v in batch.items()} if isinstance(batch, dict) else len(batch))
            break
        ...
        if hasattr(self.config.data, 'val_files') and self.config.data.val_files:
            ...
            sampler_val = SequentialSampler(data_source=self.val_dataset)
            ...
            assert len(self.val_dataloader) >= 1, "Validation dataloader must have at least one batch"
    ...
    def _validate(self):
        ...
        if not hasattr(self, 'val_dataloader') or self.val_dataloader is None:
            print("No validation dataloader found, skipping validation")
            return
        ...
        envs = [self.val_env.copy() for _ in range(val_batch_size)] if self.val_env else []

        for batch_dict in self.val_dataloader:
            timing_raw = {}
            val_batch: DataProto = DataProto.from_single_dict(batch_dict)

            # Reset environments with seeds if available
            if envs and 'extra_info' in val_batch.non_tensor_batch:
                env_seeds = [i.get('index', 0) for i in val_batch.non_tensor_batch['extra_info']]
            ...
            gen_batch.meta_info.update({
                ...
                'do_sample': True,  # Use sampling for validation
                'validate': True,
                'max_rounds': self.config.algorithm.rounds_ctrl.get('val_rounds', 
                                                                   self.config.algorithm.rounds_ctrl.rounds),
            })
            with _timer('val_step', timing_raw):
                # Generate sequences
                with _timer('val_gen', timing_raw):
                    self.actor_rollout_wg.change_to_val_mode()
                    gen_batch_output = self.actor_rollout_wg.generate_sequences(gen_batch)

                # Prepare validation batch
                val_batch.non_tensor_batch['uid'] = np.array([str(uuid.uuid4()) for _ in range(len(val_batch.batch))],
                                                             dtype=object)
                # val_batch = val_batch.repeat(repeat_times=self.config.actor_rollout_ref.rollout.n, interleave=True)
                # print(val_batch)
                # print('=========1====')
                val_batch = val_batch.union(gen_batch_output)
                # print(gen_batch_output)
                # print('===2=========')
                # print(val_batch)
                
                # Get rewards from batch (same as training)
                if 'scores' in val_batch.batch:
                    reward_tensor = val_batch.batch['scores']
                    # Sum rewards along sequence dimension to get total reward per sample
                    scores = reward_tensor.sum(dim=-1).tolist()
                    all_scores.extend(scores)
                    
                    # Compute reward metrics
                    reward_metrics = {
                        'val_reward/mean': np.mean(scores),
                        'val_reward/max': np.max(scores),
                        'val_reward/min': np.min(scores),
                        'val_reward/std': np.std(scores),
                    }
                    print(reward_metrics)
                    global_metrics.update(reward_metrics)
                else:
                    print("Warning: No 'rewards' found in validation batch")

        # Log validation metrics
        self.logger.log(data=global_metrics, step=self.global_steps)
        print(f"Validation completed at step {self.global_steps}")
        self.actor_rollout_wg.change_to_train_mode()
    ...
    def fit(self):
        ...
        #     tmp = self._validate()
        #     print('Validation before training metrics:', tmp)
                ...
                with _timer('step', timing_raw):
                    ...
                    if self.config.algorithm.adv_estimator == 'remax':
                        with _timer('gen_max', timing_raw):
                            ...
                            print(self.actor_rollout_wg.validation_mode)
                            print('====0000000======')
                            self.actor_rollout_wg.change_to_train_mode()
                    ...
                    with _timer('adv', timing_raw):
                        # Get rewards from batch directly
                        if 'rewards' in batch.batch:
                            reward_tensor = batch.batch['rewards']
                        else:
                            # Fallback to scores if rewards not available
                        ...
                        # filter data with too low rewards std
                    ...
                    # validate
                    if hasattr(self, 'val_dataloader') and self.config.trainer.test_freq > 0 and \
                        self.global_steps % self.config.trainer.test_freq == 0: 
                        with _timer('testing', timing_raw):
                            tmp = self._validate()
                            print('Validation metrics:', tmp)
