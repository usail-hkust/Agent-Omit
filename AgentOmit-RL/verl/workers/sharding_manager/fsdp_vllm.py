...
from .fp8_utils import quant_weight_to_fp8_gen
...
class FSDPVLLMShardingManager(BaseShardingManager):
    ...
    def __init__(self,
                 ...
                 fp8_infer_origin_train: bool = False):
        self.module = module
        self.inference_engine = inference_engine
        self.model_config = model_config
        self.device_mesh = device_mesh
        self.fp8_infer_origin_train = fp8_infer_origin_train
    ...
    def update_params(self, updated_params):
        ...
        if model.config.architectures[0] in ['DeepseekV2ForCausalLM', 'DeepseekV3ForCausalLM']:
            assert not self.fp8_infer_origin_train, (
                f"Dont support {model.config.architectures[0]} to use fp8_infer_origin_train."
            )
            ...
            if self.fp8_infer_origin_train:
                loaded_params = model.load_weights(
                    quant_weight_to_fp8_gen(model, updated_params, world_size != 1)
                )
            else:
                loaded_params = model.load_weights(
