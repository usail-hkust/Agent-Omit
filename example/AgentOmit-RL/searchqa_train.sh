...
bash /your_path/AgentOmit-Gym/agentenv-searchqa/start.sh ${FREE_PORT}

wait_for_service() {
    local timeout=600  # 10分钟超时
    local start_time=$(date +%s)
    echo "等待 searchqa 服务启动..."
    
    while ! netstat -tunlp | grep -q ":${FREE_PORT}"; do
        ...
        echo "服务未就绪，继续等待... (剩余${remaining}秒)"
        sleep 30
    done
...
model_save_dir="/your_path/Qwen3-8B"

exp_name="searchqa_efficient_Qwen3-8B"
...
HYDRA_FULL_ERROR=1 WANDB_MODE=online python3 -m verl.agent_trainer.main_ppo \
    ...
    algorithm.rounds_ctrl.rounds=8 \
    ...
    actor_rollout_ref.rollout.max_tokens=3072 \
