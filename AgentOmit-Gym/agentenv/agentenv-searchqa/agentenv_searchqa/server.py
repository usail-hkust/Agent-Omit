...
def step(step_query: StepQuery):
    ...
    observation, reward, done, skip_turn = searchqa_env_server.step(
        step_query.env_idx, step_query.action, step_query.idx
    )
    # print(f"Observation: {observation}")
    return StepResponse(observation=observation, reward=reward, done=done, skip_turn=skip_turn)
