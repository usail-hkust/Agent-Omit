...
# sciworld_sys_prmompt = """You are an agent for science world. Every round I will give you an observation, you have to respond an action based on the observation to finish the given task. Here are the actions you may take: [{"action": "open OBJ", "description": "open a container"}, {"action": "close OBJ", "description": "close a container"}, {"action": "activate OBJ", "description": "activate a device"}, {"action": "deactivate OBJ", "description": "deactivate a device"}, {"action": "connect OBJ to OBJ", "description": "connect electrical components"}, {"action": "disconnect OBJ", "description": "disconnect electrical components"}, {"action": "use OBJ [on OBJ]", "description": "use a device/item"}, {"action": "lookaround", "description": "describe the current room"}, {"action": "lookat OBJ", "description": "describe an object in detail"}, {"action": "lookin OBJ", "description": "describe a container\'s contents"}, {"action": "read OBJ", "description": "read a note or book"}, {"action": "move OBJ to OBJ", "description": "move an object to a container"}, {"action": "pickup OBJ", "description": "move an object to the inventory"}, {"action": "putdown OBJ", "description": "drop an inventory item"}, {"action": "pour OBJ into OBJ", "description": "pour a liquid into a container"}, {"action": "dunk OBJ into OBJ", "description": "dunk a container into a liquid"}, {"action": "mix OBJ", "description": "chemically mix a container"}, {"action": "goto LOC", "description": "move to a new location"}, {"action": "eat OBJ", "description": "eat a food"}, {"action": "flush OBJ", "description": "flush a toilet"}, {"action": "focuson OBJ", "description": "signal intent on a task object"}, {"action": "wait", "description": "take no action for 10 iterations"}, {"action": "wait1", "description": "take no action for 1 iteration"}, {"action":"examine OBJ","description":"provides a description of the objects present on or in a receptacle."}, {"action": "task", "description": "describe current task"}, {"action": "inventory", "description": "list your inventory"}]
...
# Reminder:
# 1. An action should be wrapped in <tool_call>...</tool_call>, and the action must be chosen from the given functions. The objects you choose must exist in the current room. Any actions except provided available actions will be regarded as illegal. 
...
#     "use", "lookaround", "lookat", "lookin", "read", "move", 
#     "pickup", "putdown", "pour", "dunk", "mix", "goto", "eat", 
#     "flush", "focuson", "wait", "wait1", "examine", "task", "inventory"
# ]
...
efficient_sciworld_sys_prompt = """You are an agent for science world. Every round I will give you an observation, you have to respond an action based on the observation to finish the given task. Here are the actions you may take: [{"action": "open OBJ", "description": "open a container"}, {"action": "close OBJ", "description": "close a container"}, {"action": "activate OBJ", "description": "activate a device"}, {"action": "deactivate OBJ", "description": "deactivate a device"}, {"action": "connect OBJ to OBJ", "description": "connect electrical components"}, {"action": "disconnect OBJ", "description": "disconnect electrical components"}, {"action": "use OBJ [on OBJ]", "description": "use a device/item"}, {"action": "look around", "description": "describe the current room"}, {"action": "look at OBJ", "description": "describe an object in detail"}, {"action": "look in OBJ", "description": "describe a container\'s contents"}, {"action": "read OBJ", "description": "read a note or book"}, {"action": "move OBJ to OBJ", "description": "move an object to a container"}, {"action": "pick up OBJ", "description": "move an object to the inventory"}, {"action": "put down OBJ", "description": "drop an inventory item"}, {"action": "pour OBJ into OBJ", "description": "pour a liquid into a container"}, {"action": "dunk OBJ into OBJ", "description": "dunk a container into a liquid"}, {"action": "mix OBJ", "description": "chemically mix a container"}, {"action": "go to LOC", "description": "move to a new location"}, {"action": "eat OBJ", "description": "eat a food"}, {"action": "flush OBJ", "description": "flush a toilet"}, {"action": "focus on OBJ", "description": "signal intent on a task object"}, {"action": "wait", "description": "take no action for 10 iterations"}, {"action": "wait1", "description": "take no action for 1 iteration"}, {"action":"examine OBJ","description":"provides a description of the objects present on or in a receptacle."}, {"action": "task", "description": "describe current task"}, {"action": "inventory", "description": "list your inventory"}]
...
actions = [
    "open", "close", "activate", "deactivate", "connect", "disconnect", 
    "use", "look around", "look at", "look in", "read", "move", 
    "pick up", "put down", "pour", "dunk", "mix", "go to", "eat", 
    "flush", "focus on", "wait", "wait1", "examine", "task", "inventory"
]
...
class SciWorldAdapter(BaseAdapter):
            ...
            ConversationMessage(
                {
                    "from": "human",
                    "loss": None,
                    "value": efficient_sciworld_sys_prompt,
                }
            ),
        ),
    ...
    def parse_react(text: str) -> ActionWithTought:
        ...
        if len(_split) == 2:
            if any(action in _split[1].lower() for action in actions):
...
class SciworldEnvClient(BaseEnvClient):
    ...
    def step(self, payload_str: str, idx) -> StepOutput:
        ...
        omit_tool_response_num = self.adapter_cls.parse_tool_response_skip(action)
        ...
        try:
            ...
            print(e, action)
            return StepOutput(
                state=f"<tool_response_{self.turn}>Invalid Action." + self.observe()+ f"</tool_response_{self.turn}>", reward=0.0, done=False
            )

        response = self._post("step", {"action": action})
]
        observation = response['observation']
        formatted_observation = f"<tool_response_{self.turn}>{observation}</tool_response_{self.turn}>"
        ...
        normalized_reward = self.normlize_score(response["reward"])
        print('=========')
        ...
        self.info = {
            ...
            "reward": (1-self.reward_reweight)*normalized_reward + self.reward_reweight* self.reward_delta,
        ...
        return StepOutput(
            ...
            reward=normalized_reward + self.reward_delta,
            done=response["done"],
            skip_turn=omit_tool_response_num,
        )

    def normlize_score(self, reward: float) -> float:
        if reward <= 0:
            normalized_reward = 0
        else:
            # 使用log函数映射，这里假设reward是正数
            # log(1+reward)的方式可以将正数映射到(0, +∞)
            normalized_reward = 1 - 1 / (1 + math.log(1 + abs(reward)))
